...
const fetchUser = async () =>{
    const {data} =await axiosIntance.get("/Mentor/ProfileTakeMentor")
    if(data){
        console.log(data);
       const {FoundedMentor} = data 
       setUser(FoundedMentor)
    }
}
const handleLogout=()=>{
    const Mentor= localStorage.removeItem("Mentor")
...
//   console.log(formData,"__________________________");
   ...
   const handleSubmit = async() =>{
    const {data} = await axiosIntance.post("Mentor/UpdateMentorProfile",{ImagePreviewProfile,UpdateUsername})
    if(data){
        console.log(data);
